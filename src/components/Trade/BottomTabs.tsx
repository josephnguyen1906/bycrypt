...
export default function BottomTabs({
  ...
  onPerpChanged?: () => void;
  listTab?: number;
  onListTabChange?: (tab: number) => void;
}) {
  ...
  const tab = listTab ?? innerTab;
  const setTab = (value: number) => {
    onListTabChange?.(value);
    if (listTab === undefined) setInnerTab(value);
  };
    ...
    const timer = setInterval(() => {
      setTimeLeft((prev) => {
        ...
        Object.keys(prev).forEach((id) => {
          ...
          updated[numId] = prev[numId] > 0 ? prev[numId] - 1 : 0;
        });
        return updated;
      });
    }, 1000);
  ...
  const renderContractOrder = (item: IHistoryOpen, index: number) => (
    <Box
      key={item.id ?? index}
      sx={{
        width: "100%",
    ...
    >
      ...
      >
        <Box>
          ...
          <Typography sx={{ color: "#9aa4b2", fontSize: 13, mt: "5px" }}>
        ...
        <Box sx={{ textAlign: "right" }}>
          ...
          >
            {item.hyzd === 1 ? t("TradePage.title6") : t("TradePage.title7")}
          </Typography>
          <Typography sx={{ color: "#9aa4b2", fontSize: 13, mt: "5px" }}>
